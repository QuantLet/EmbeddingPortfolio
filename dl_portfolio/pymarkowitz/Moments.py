...
class MomentGenerator:
    ...
    def __init__(self, return_data: Union[pd.DataFrame, np.ndarray], assets: Optional[List[str]] = None):
        """
        ...
        if isinstance(return_data, pd.DataFrame):
            ...
            self.return_mat = return_data.transpose()
            self.assets = assets
        else:
    ...
    def calc_cov_mat(self, technique='sample', semi=False, method='default', time_scaling=252,
                     builtin=False, weights=None, bm_return=0.00025, assume_zero=False, normalize=False,
                     ret_format='df', **kwargs):
        """
    ...
    def calc_beta(self, beta_vec, technique="sample", semi=False, method='default', builtin=False, weights=None,
                  ret_format='series', **kwargs):
        """
    ...
    def calc_coskew_mat(self, semi=False, method='default', weights=None, bm_return=0.001, assume_zero=False,
                        normalize=True, ret_format='raw', **kwargs):
        ...
        """
        return self.calc_comoment_mat(moment=3, semi=semi, method=method, weights=weights, bm_return=bm_return,
                                      assume_zero=assume_zero, normalize=normalize, ret_format=ret_format, **kwargs)

    def calc_cokurt_mat(self, semi=False, method='default', weights=None, bm_return=0.001, assume_zero=False,
                        normalize=True, ret_format='df', **kwargs):
        ...
        return self.calc_comoment_mat(moment=4, semi=semi, method=method, weights=weights, bm_return=bm_return,
                                      assume_zero=assume_zero, normalize=normalize, ret_format=ret_format,
                                      **kwargs) * bias_factor

    def calc_comoment_mat(self, moment, semi=False, method='default', weights=None, bm_return=0.001, assume_zero=False,
                          normalize=True, ret_format='raw', **kwargs):
        ...
        *Calling this method to calculate covariance matrix by setting moment=2 will yield identical result
        ...
        if ret_format == 'df':
            return pd.DataFrame(comoment_mat, index=self.assets,
                                columns=tuple(itertools.product(*[self.assets for i in range(moment - 1)])))
        elif ret_format == 'raw':
            return comoment_mat
        else:
            raise FormatException("Invalid Format. Valid options are: df, raw")
    ...
    def calc_moment_mat(moment, return_mat, weight_mat, normalize):
        ...
        unbias_factor = np.prod(np.repeat(num_obs, moment - 1) / (num_obs - np.arange(1, moment,
                                                                                      1)))  # (num_obs ** (moment - 1)) / (np.prod(num_obs - np.arange(1, moment, 1)))
