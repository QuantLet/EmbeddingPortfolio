...
def wrap_function(function, args):
    ...
    def function_wrapper(*wrapper_args):
        ncalls[
            0] += 1  # record the number of times that the function is called.
...
def ALMfunc(x, objFun, conFuns, w, v, sigma):
    ...
    if (len(ineqConFuns) != len(w)) or (len(eqConFuns) != len(v)):
        ...
        exit(-1)
    sumValue1 = np.sum(
        [(np.max([0, wi - sigma * ineqi])) ** 2 - wi ** 2 for wi, ineqi in
         zip(w, ineqConFuns)])
    sumValue2 = np.sum([0.5 * sigma * ((eqj) ** 2) - vj * eqj for vj, eqj in
                        zip(v, eqConFuns)])
...
def _fmin_ALM(objFun, x0, conFuns, eqConNum, ineqConNum, \
    ...
    conFunc_calls, conF = wrap_function(conFuns,
                                        conArgs)  # conf in each list returned by conF(x) is callable
    allvecs = []
    ...
    if ineqConNum == 0:
        ...
        wk = multiplierRange[0] + (multiplierRange[1] \
                                   - multiplierRange[0]) * np.random.rand(
            ineqConNum)
    if eqConNum == 0:
        vk = np.array([])
    else:
    ...
    while (gnorm > gtol) and (k < maxiter):
        ...
        if gnorm < gtol:
            print(
                """The equality constraint norm evaluated at x[%i] is less than the global tolerance!""" % k)
            break
        # check the objective value
        if not np.isfinite(fval):
            print(
                """The function value evaluated at x[%i] is infinite! Something may go wrong!""" % k)
            warnflag = 2
            break
        if retall:
    ...
    result = OptimizeResult(fun=fval, nfev=func_calls[0],
...
if __name__ == "__main__":
    ...
    ineqParameters = [(1, -2, 2), (-1, -2, 6), (-1, 2, 2), (1, 0, 0),
                      (0, 1, 0)]
    ...
    res = fmin_ALM(testObjFun, x0, testConFun, eqConNum=1, ineqConNum=5, \
                   funArgs=(funParameters,), conArgs=(conParaDict,),
