...
def body(prev_A, prev_B, prev_SR, batch, i):
    prev_A = prev_A + ETA * (batch[i] - prev_A)
    prev_B = prev_B + ETA * (batch[i] ** 2 - prev_B)
    D = diff_sr(prev_A, prev_B, batch[i])
    prev_SR = prev_SR + ETA * D
    return prev_A, prev_B, prev_SR, batch, i + 1


def cond(prev_A, prev_B, prev_SR, batch, i):
    return i < batch.shape[0]


def diff_sharpe_ratio(prev_A, prev_B, prev_SR, port_returns: tf.Tensor, benchmark: float = 0.0093):
    port_returns = port_returns - tf.constant(benchmark, dtype=tf.float32)
    prev_A, prev_B, prev_SR, _, _ = tf.while_loop(cond=cond,
                                                  body=body,
                                                  loop_vars=(prev_A, prev_B, prev_SR, port_returns, 0),
                                                  parallel_iterations=1)
    return prev_A, prev_B, - prev_SR
...
if __name__ == '__main__':
    ...
    parser.add_argument("--wait-period", type=int, default=2,
                        help="Minimum nb of batches to initilize diff sharpe ratio")
    parser.add_argument("--eta", type=float, default=0.01,
                        help="Exponential smoothing factor for diff sharpe ratio")
    ...
    optimizer = tf.keras.optimizers.SGD(learning_rate=args.learning_rate, momentum=args.momentum, clipvalue=5)

    # Training loop
    LOGGER.info('Start main training loop ...')
    weights = []
    ...
    for epoch in range(args.n_epochs):
        ...
        LOGGER.info(f'Initialising first and second moment on {args.wait_period} batches')
        for features, returns in train_dataset:
            ...
            if counter < args.wait_period:
                ...
                for i in range(port_return.shape[0]):
                    A = prev_A + args.eta * (port_return[i] - prev_A)
                    B = prev_B + args.eta * (port_return[i] ** 2 - prev_B)
                prev_A = A
                prev_B = B
            else:
                if counter % 100 == 0:
                    LOGGER.info(f'Batch to go: {nb_batch - counter} for epoch {epoch}')
                # Optimize the model
                with tf.GradientTape() as tape:
                    ...
                    port_return_no_fee, port_return = portfolio_returns(actions, returns, initial_position,
                                                                        trading_fee=args.trading_fee,
                                                                        cash_bias=not args.no_cash)
                    prev_A, prev_B, loss_value = diff_sharpe_ratio(prev_A, prev_B, loss_value, port_return,
